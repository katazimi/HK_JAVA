...
<script type="text/javascript">
	function updateRole(button, id) {
		...
		location.href = "updateRole.admin?id="+id+"&role="+role;
	}
</script>
</head>
<body>
					...
					<c:otherwise>
						<c:forEach items="${list}" var="dto">
							<tr>
								...
								<td> 
									<select name="status">
									  <option value="USER" ${dto.role eq 'USER' ? 'selected' : ''}>USER</option>
									  <option value="MANAGER" ${dto.role eq 'MANAGER' ? 'selected' : ''}>MANAGER</option>
									  <option value="ADMIN" ${dto.role eq 'ADMIN' ? 'selected' : ''}>ADMIN</option>
									</select>
			      					<button onclick="updateRole(this, '${dto.id}')">수정</button>
			      				</td>
								<td>${dto.regdate}</td>
							</tr>
						</c:forEach>
